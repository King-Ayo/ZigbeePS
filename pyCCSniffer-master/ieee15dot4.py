...
class SFS(object):
    def __init__(self, beaconOrder, superframeOrder, finalCAPSlot, ble,
        ...
        dic["BeaconOrder"] = beaconOrder
        dic["SuperFrameOrder"] = superframeOrder
...
class SimpleAddress(object):
    def __init__(self, panId, simpleAddress):
        ...
        dic["Destination_PAN"] = "{:x}".format(panId)
        dic["Destination_SimpleAddr"] = "{:x}".format(simpleAddress)

    def __repr__(self, *args, **kwargs):
        return "PAN[{:x}] SimpleAddr[{:x}]".format(self.panId, self.address)

class SimpleAddress2(object):
    def __init__(self, panId, simpleAddress):
        self.panId = panId
        self.address = simpleAddress
        dic["Source_PAN"] = "{:x}".format(panId)
        dic["Source_SimpleAddr"] = "{:x}".format(simpleAddress)
...
class ShortAddress(object):
    def __init__(self, panId, shortAddress):
        ...
        dic["Destination_PAN"] = "{:x}".format(panId)
        dic["Destination_ShortAddr"] = "{:x}".format(shortAddress)

    def __repr__(self, *args, **kwargs):
        return "PAN[{:x}] ShortAddr[{:x}]".format(self.panId, self.address)


class ShortAddress2(object):
    def __init__(self, panId, shortAddress):
        self.panId = panId
        self.address = shortAddress
        dic["Source_PAN"] = "{:x}".format(panId)
        dic["Source_ShortAddr"] = "{:x}".format(shortAddress)
...
class ExtendedAddress(object):
    def __init__(self, panId, extAddress):
        ...
        dic["Destination_PAN"] = "{:x}".format(panId)
        dic["Destination_EXTAddr"] = "{:x}".format(extAddress)

    def __repr__(self, *args, **kwargs):
        return "PAN[{:x}] ExtAddr[{:x}]".format(self.panId, self.address)
    

class ExtendedAddress2(object):
    def __init__(self, panId, extAddress):
        self.panId = panId
        self.address = extAddress
        dic["Source_PAN"] = "{:x}".format(panId)
        dic["Source_EXTAddr"] = "{:x}".format(extAddress)
...
class IEEE15dot4Frame(object):
    def __init__(self, timestamp, fcf, sequenceNumber, addressing, msdu, *args,
        ...
        self.msdu = msdu
        dic["FrameType"] = FrameType.toString(fcf.frametype)
        x = binascii.hexlify(msdu)
        dic["MSDU"] = x.decode("utf-8")
...
class IEEE15dot4AckFrame(IEEE15dot4Frame):
    def __init__(self, *args, **kwargs):
        ...
        dic["FrameType"] = FrameType.toString(self.fcf.frametype)
        dic["SeqNum"] = self.sequenceNumber
        dic["TimeStamp"] = datetime.now().timestamp()
...
class IEEE15dot4FrameFactory(object):
    ...
    def parse(packet):
        ...
        if fcf.frametype is FrameType.ACK:
            return IEEE15dot4AckFrame(**frame.__dict__),dic
        elif fcf.frametype is FrameType.BEACON:
            return IEEE15dot4FrameFactory.__parseBeacon(frame),dic
        elif fcf.frametype is FrameType.MAC_CMD:
            return IEEE15dot4FrameFactory.__parseMACCommand(frame),dic
