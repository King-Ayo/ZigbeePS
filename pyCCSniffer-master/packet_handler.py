    ...
    def handleSniffedPacket(self, sniffedPacket):
        ...
        try:
            ...
            if customFrame:
                ...
                frame,dic = ieee.IEEE15dot4FrameFactory.parse(sniffedPacket)
                capture = CapturedFrame(frame, rssiSniff, self.__annotation)
                dic["RssiSniff"] = rssiSniff
                pdic = {"Measurments": "Zigbee", "Fields": dic}
                pdic["Timestamp"] = pdic["Fields"]["TimeStamp"]
                del pdic["Fields"]["TimeStamp"]
                self.cap = pdic

                if capture:
                    self.queue.append(pdic)
                    self.captures.append(capture)
                    print(capture)
                    # hack here!
                    sys.stdout.flush()
